...
def send_startup_intro():
    intro = (
        "🤖 **Nawthviper Scalper Bot Active**\n\n"
        "📌 *How We Trade:* Candle Range Theory (CRT) + candlestick patterns + indicator filters.\n"
        "Multiple confirmations determine trade quality.\n\n"
        "📊 *Confidence Levels:*\n"
        "   🔥 High (≥ 0.85): Strong signals – recommended to follow.\n"
        "   ⚖️ Medium (0.70–0.84): Moderate signals – trade with caution.\n"
        "   ❗ Low (0.60–0.69): Borderline setups – mainly for observation.\n\n"
        "🕒 *Active Sessions:*\n"
        "   • London: 07:00–16:00 UTC (GBPUSD, USDCHF, EURUSD)\n"
        "   • New York: 12:00–21:00 UTC (USDJPY, XAUUSD, USDCAD)\n"
        "   • Asia: 00:00–09:00 UTC (JPY, AUD, NZD)\n\n"
        "✅ *Trading Rules:*\n"
        "   • Only High Confidence signals are recommended.\n"
        "   • Medium signals are optional for aggressive strategies.\n"
        "   • Low signals are for learning/observation only.\n\n"
        "⚠️ *Disclaimer:* ⚠️\n"
        "I am not a financial advisor. All trades carry risk. "
        "Always research before trading and never risk more than you can afford to lose."
...
def has_conflict_and_existing_confidence(symbol, side):
    ...
    # No active runtime record → check live positions
    positions = get_live_positions(symbol)
    if not positions:
        return False, None
    ...
    for pos in positions:
        pos_side = 'buy' if getattr(pos, 'type', None) == mt5.ORDER_TYPE_BUY else 'sell'
        if pos_side != side:
            return True, None  # Opposite-side conflict
        if pos_side == side and not ALLOW_PYRAMID_SAME_SIDE:
            return True, None  # Pyramid disabled
    return False, None
...
def monitor_and_trade(symbol, strategy_mode=None, fixed_lot=None):
    ...
    # --- Session filter ---
    if not handle_global_session():
        send_info(f"[SESSION FILTER] {symbol} → Not in active session, skip trading.")
    ...
    for sig in signals:
        try:
            ...
            # --- Risk-based lot override: compute using entry/sl from the signal
            # Default risk_percent = 1.0 (1% of equity). Change as desired.
            try:
            ...
            if confidence < dynamic_conf:
                continue
            # adaptive_adx may have not been defined in this scope if ADX calculation failed earlier;
            # guard that usage to avoid NameError.
            try:
                send_info(f"[ADAPT] {symbol} adaptive_ADX={adaptive_adx:.2f}, adaptive_conf={dynamic_conf:.2f}")
            except Exception:
                send_info(f"[ADAPT] {symbol} adaptive_conf={dynamic_conf:.2f}")
            ...
            if conflict:
                ...
                if AUTO_CLOSE_ON_STRONG and existing_conf is not None:
                    if confidence >= existing_conf + CLOSE_CONF_DIFF:
                        # Close all opposite-side positions first
                        close_positions_for_symbol(symbol, reason_msg=f"auto_close_by_conf_{confidence:.2f}")
                        active_trades_by_symbol.pop(symbol, None)

                        # Wait until opposite positions are gone
                        wait_attempts = 0
                        while True:
                            live_pos = get_live_positions(symbol)
                            opposite_exists = any(
                                ('buy' if side == 'sell' else 'sell') ==
                                ('buy' if getattr(p, 'type', None) == mt5.ORDER_TYPE_BUY else 'sell')
                                for p in live_pos
                            )
                            if not opposite_exists or wait_attempts > 10:
                                break
                            time_module.sleep(0.3)  # wait 300ms
                            wait_attempts += 1
                    else:
                        # Signal not strong enough to override existing opposite-side positions
                        continue
                else:
                    # Either same-side conflict and pyramiding not allowed, or auto-close not enabled
                    if not ALLOW_PYRAMID_SAME_SIDE:
                        continue
        # else: same-side pyramid allowed → continue to place trade
