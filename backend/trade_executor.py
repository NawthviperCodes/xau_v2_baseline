...
def close_partial_and_move_sl_to_be(ticket, partial_percent=0.5):
    with EXECUTION_LOCK:
        ...
        symbol = pos.symbol
        vol_close = round(pos.volume * partial_percent, 2) # Simplify rounding
        ...
        # 2. Move SL to Entry
        time.sleep(0.2) # Small wait for server update
...
def place_order(symbol, side, lot, magic, comment="", sl=None, tp=None):
    with EXECUTION_LOCK:
        ...
        request = {
            ...
            "type_filling": mt5.ORDER_FILLING_IOC, # Changed to IOC for speed
        }
        ...
        if result.retcode != mt5.TRADE_RETCODE_DONE:
            print(f"[Exec] Error: {result.comment}")
            return None
...
def trail_sl(symbol, magic, trail_pips=150):
    # This reads positions (safe) and sends orders (locked)
    # Optimization: Check if update needed before acquiring lock
...
def close_positions_for_symbol(symbol):
    ...
    Used for emergency stops or auto-closing opposite trades.
    """
    with EXECUTION_LOCK:
        positions = mt5.positions_get(symbol=symbol)
        if not positions: 
        ...
        for pos in positions:
            ...
            if res.retcode != mt5.TRADE_RETCODE_DONE:
                print(f"[Exec] Failed to close {pos.ticket}: {res.comment}")
            else:
