...
def move_to_breakeven(symbol, magic, be_trigger_ratio=0.5):
    """
    Moves SL to breakeven (entry + spread) when profit reaches a percentage of TP distance.
    """
    ...
    for pos in positions:
        if pos.magic != magic or pos.sl != 0: # Only target trades without an existing SL
            continue
        ...
        # Calculate distance to breakeven (entry + small buffer for spread/commission)
        spread_points = abs(tick.ask - tick.bid) / symbol_info.point
        breakeven_sl = entry + direction * symbol_info.point * spread_points * 2 
        
        # Calculate trigger distance
        tp_distance = abs(tp - entry)
        profit_distance = abs((tick.bid if direction == 1 else tick.ask) - entry)
        
        # Check if profit has reached the BE trigger (e.g., 50% of TP distance)
        if profit_distance >= tp_distance * be_trigger_ratio:
            
            # Ensure the new SL is better than the original (to avoid broker error)
            if (direction == 1 and breakeven_sl > pos.sl) or \
               (direction == -1 and breakeven_sl < pos.sl):
                ...
                if result and result.retcode == mt5.TRADE_RETCODE_DONE:
                    msg = f"✅ SL moved to Breakeven for {symbol} | Ticket: {pos.ticket}"
                    print(msg)
...
def place_order(symbol, order_type, lot, sl_price, tp_price, magic_number, comment="auto-trade"):
   ...
   # --- NEW: Safety check to ensure broker's stops_level is respected ---
    min_dist_point = stops_level * 1.2

    # Respect broker's minimum stop distance, but trust the strategy's ATR-based SL/TP
    if order_type == "buy":
        if (price - sl_price) < min_dist_point:
            sl_price = price - min_dist_point
            print(f"[WARN] Adjusted SL for BUY {symbol} to {sl_price:.5f} due to broker min distance.")
        if (tp_price - price) < min_dist_point:
            tp_price = price + min_dist_point
            print(f"[WARN] Adjusted TP for BUY {symbol} to {tp_price:.5f} due to broker min distance.")

    elif order_type == "sell":
        if (sl_price - price) < min_dist_point:
            sl_price = price + min_dist_point
            print(f"[WARN] Adjusted SL for SELL {symbol} to {sl_price:.5f} due to broker min distance.")
        if (price - tp_price) < min_dist_point:
            tp_price = price - min_dist_point
            print(f"[WARN] Adjusted TP for SELL {symbol} to {tp_price:.5f} due to broker min distance.")
